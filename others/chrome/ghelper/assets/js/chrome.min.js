...
function notify2(title, body = null, url = null) {
    var options = {};
    if (body) {
        options['body'] = body;
    }
    msg = new Notification(title, options);
    if (url) {
...
function set_proxy(proxy_pac_key, conf) {
    chrome.proxy.settings.set(generate_pac(conf), function () {
...
function get_local_data() {
    var data = {};
    data['gtoken'] = get_token();
    data['version'] = manifest.version;
    data['proxy_pac_key'] = get_proxy_pac_key();
    data['api_url'] = get_api_url();
    data['lang'] = get_client_lang();
...
function purl(path, up = false) {
    if (up) {
        var api_url = api_urls.shift();
        set_api_url(api_url);
        api_urls.push(api_url);
    }
    var api_url = get_api_url();
    if (!api_url) {
        return purl(path, true);
    }
    var query = get_local_data();
    var pquery = getQueryVars(path);
    for (k in pquery) {
        query[k] = pquery[k];
    }
    path = path.split('?')[0];
...
function handle(data) {
    if (data.headers) {
        ...
        if (data.headers.open_tab) {
            window.open(data.headers.open_tab.url, 'ghelper');
        }
...
function session(action = 'reload', async = true, up = false, tn = 1) {
    ...
    conf['success'] = function (res) {
        handle(res);
        data = res;
        localStorage.setItem('session_sync', get_time());
    }
...
function error_html(err) {
    ...
    html += '<h5><a href="error.html" target="_blank" class="btn btn-warning">' + getlang('error_button') + '</a></h5>';
    html += '</div></div>';
    $('body').html(html);
}
...
function check_proxy_config(show = 0) {
    chrome.proxy.settings.get({'incognito': false}, function (config) {
        if (config['levelOfControl'] != 'controlled_by_this_extension') {
            if (show) {
                $('#alert-messager').html('<span class="label label-danger"><i class="glyphicon glyphicon-exclamation-sign"></i>' + getlang('check_proxy_config_title') + '</span>' + getlang('check_proxy_config_body'));
            } else {
                $('#alert-messager').html('<span class="label label-danger"><i class="glyphicon glyphicon-exclamation-sign"></i>' + getlang('check_proxy_config_title') + '</span>');
            }
        }
    });
}

function cancel_urls(details) {
    return {cancel: true};
}

function set_block_urls(block_urls) {
    chrome.webRequest.onBeforeRequest.addListener(cancel_urls, {urls: block_urls}, ["blocking"]);
    localStorage.setItem('adblock', 'on');
}

function remove_block_urls() {
    chrome.webRequest.onBeforeRequest.removeListener(cancel_urls);
}

function message_handler(request, sender, sendResponse) {
    if (request == 'remove_block_urls') {
        remove_block_urls();
    }
}

function tabs_updated_handler(id, status, tab) {
    if (!tabs_data.hasOwnProperty(id)) {
        tabs_data[id] = []
    }
}

function tabs_removed_handler(id, info) {
    if (tabs_data.hasOwnProperty(id)) {
        delete tabs_data[id];
    }
}

function brower_urls_handler(details) {
    var id = details.tabId;
    if (tabs_data.hasOwnProperty(id)) {
        tabs_data[id].push(details);
    }
}
